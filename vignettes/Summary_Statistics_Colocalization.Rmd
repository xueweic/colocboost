...
- `n`: sample size for the summary statistics, it is highly recommended to provide.
...
for (i in 1:length(X)){
  ...
  y <- Y[[i]]
  effect_n[i] <- length(y)
  rr <- susieR::univariate_regression(X = x, y = y)
  effect_est <- cbind(effect_est, rr$betahat)
  effect_se <- cbind(effect_se, rr$sebetahat)
}
