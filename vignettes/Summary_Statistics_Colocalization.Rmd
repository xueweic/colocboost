...
When the number of variants P is very large, storing the full P x P LD matrix may be infeasible.
If you have the reference panel genotype matrix from which LD would be computed, you can pass it directly via `X_ref`.
ColocBoost will compute LD products on the fly, avoiding the P x P memory cost.

This is beneficial when the reference panel sample size (N_ref) is less than the number of variants (P).
When N_ref >= P, ColocBoost automatically precomputes the LD matrix internally for efficiency.

Provide either `LD` or `X_ref`, not both. The `dict_sumstatLD` dictionary works with `X_ref` the same way as with `LD`.
