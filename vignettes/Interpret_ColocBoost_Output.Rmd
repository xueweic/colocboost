...
In this section, we will provide a detailed explanation of the interest components for deepening into ColocBoost result using a mixed dataset.
...
To plot the VCP for the variants with in at least one CoS, you can use the `colocboost_plot` function with the `y` argument set to `"vcp"`. (please visit our documentation portal 
...
- **`n_variables`** is the regression coefficients estimated from the colocboost model for each trait.
- **`variables`** is vector of variable names across all traits being included in the colocalization analysis.
- **`coef`** is the regression coefficients estimated from the colocboost model for each trait.
- **`z`** is the z-scores from marginal associations for each trait.
- **`n_outcomes`** is the regression coefficients estimated from the colocboost model for each trait.
- **`outcome_info`** contains the information of analyzed data, including sample size and data type.
...
- **`cos_details`**: A object with all information for colocalization results.
  - `cos`: A data frame with detailed information for each colocalization event.
    - `cos_index`: Indices of colocalized variables.
    - `cos_variables`: Names of colocalized variables.
  - `cos_outcomes`: 
    - `outcome_index`: 
    - `outcome_variable`: 
  - `cos_vcp`:
...
### 4.4. Model information (**`model_info`**)
