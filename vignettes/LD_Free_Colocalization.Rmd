...
The `colocboost` assumes the LD matrix accurately estimate the correlations among variants from the original GWAS genotype data.
Typically, the LD matrix comes from some public databased of genotypes in suitable reference population. 
The inaccurate LD matrix may lead to unreliable colocalization results, especially, if the LD matrix is much different from the one estimated from original genotype data.
...
ColocBoost provides the diagnostic warnings for assessing consistency of the summary statistics with the reference LD matrix.
...
The warnings serve as a diagnostic tool to alert users about potential inconsistencies in the input data.
...
with one causal per trait per region assumption:

- **One iteration approach** (recommended): performing only 1 iteration of gradient boosting with LD matrix ensures that
