...
This approach directly uses the raw data to identify causal variants.
...
This section demonstrates fine-mapping analysis using summary statistics and a proper LD matrix. 
...
In scenarios where LD information is unavailable, FineBoost can still perform fine-mapping under the assumption of a single causal variant. 
This approach is less computationally intensive but assumes that only one variant in a region is causal.
