...
There are two options avaiable for plotting the trait-specific (uncolocalized) variants:

- `plot_ucos = FALSE` (default), if `TRUE` will plot all trait-specific (uncolocalized) sets.
- `plot_ucos_idx = NULL` (default) indicates all confidence sets are plotted. `plot_cos_idx = 1` can be specified to plot the 1st confidence sets, and so on.
...
For comprehensive tutorials on result interpretation, please please visit our documentation portal 
...
colocboost_plot(res, plot_focal_cos_outocme_only = TRUE)
```
