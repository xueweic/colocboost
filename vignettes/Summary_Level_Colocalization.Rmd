...
This vignette demonstrates how to perform multi-trait colocalization analysis using summary statistics data in ColocBoost, 
...
The dataset is specifically designed for evaluating and demonstrating the capabilities of ColocBoost in multiple trait colocalization analysis with summary association data.
...
- `n`: sample size for the summary statistics, it is highly recommendation to provide.
...
- `LD` is a matrix of linkage disequilibrium (LD) information for all variants across all traits.
- The LD matrix should contain all variants present in the summary statistics data frames.
...
for different subsets of summary statistics, LD comes from different population.

- **Input Format**:
    - `sumstat` is a list of data.frames for all traits.
    - `LD` is a list of LD matrices.
