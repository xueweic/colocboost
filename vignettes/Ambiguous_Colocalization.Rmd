...
Specifically, we will use the `Ambiguous_Colocalization`, which is output from `colocboost` analyzing GTEx relsease v8 and UK Biobank summary statistics
...
- Two hits are corresponding to variants with highest PIP in SuSiE for eQTL and GWAS, respectively.

Note that the COLOC results are directly from two `susie` output objects (code provided below), which is not only strictly for only the overlapping variants between GWAS and eQTL.
...
if researcher decides to investigate these ambigous colocalization events. 
- As such,
    ...
    - The `colocboost_plot` function will not consider it as colocalized but still showing them as uncolocalized events, with overlapping variants color labelled.
