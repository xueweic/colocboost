    ...
    if (length(coloc_csets) != 0){
        ...
        if (!is.null(interest_outcome)){
          ...
          if (all(sapply(tmp, length)!=0)){
            ...
            if (sum(if.interest) == 0){ warnings("No colocalization with interest outcomes.") }
          } else {
...
get_strong_colocalization <- function(cb_output, 
  ...
  # - resummary results
  cos_re_idx <- lapply(int_weight, function(w){ unlist(get_in_cos(w, coverage = coverage))})
  cos_re_var <- lapply(cos_re_idx, function(idx){ cb_output$data_info$variables[idx] })
...
get_cos_details <- function(cb_obj, coloc_out, data_info = NULL){
  ...
  if (length(coloc_sets)!=0){
    ...
    # - resummary results
    cos_re_idx <- lapply(int_weight, function(w){ unlist(get_in_cos(w, coverage = cb_obj$cb_model_para$coverage))})
