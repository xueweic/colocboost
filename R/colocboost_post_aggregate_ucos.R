...
colocboost_post_aggregate_ucos <- function(cb_obj_single,
                                           coverage = 0.95,
                                           check_null = 0.1,
    ...
    if (sum(temp) == 0){
            ...
            if (overlap){
                ...
                if (sum(is.between) != 0){
                    ...
                    for (i.m in 1:length(potential_merged)){
                        ...
                        # define merged set
                        confidence_sets_merged <- c(confidence_sets_merged,
                        ...
                        evidence_strength_merged <- c(evidence_strength_merged,
        ...
        if (length(confidence_sets) != 0){
            ...
            for(ee in 1:length(confidence_sets)){
                ...
                purity =
                    rbind(purity,
                          matrix(get_purity(pos,X=cb_data$data[[1]]$X, Xcorr=cb_data$data[[1]]$XtX,
                                            N=cb_data$data[[1]]$N,n=n_purity),1,3))
            }
            purity = as.data.frame(purity)
            colnames(purity) = c("min_abs_corr","mean_abs_corr","median_abs_corr")
